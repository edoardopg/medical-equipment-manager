...
def send_reset_email(email, token):
    enlace = f"{os.getenv('FRONTEND_URL')}/reset-password.html?token={token}" #crea el enlace con el token para restablecer contraseña
    mensaje = MIMEText(f"""
...
def send_verification_email(email, token):
    enlace = f"{os.getenv('FRONTEND_URL')}/verify-email.html?token={token}"
    mensaje = MIMEText(f"""
Hola, gracias por registrarte.

Para verificar tu email haz clic aquí:
{enlace}

Expira en 30 minutos.
""")
    mensaje["Subject"] = "Verifica tu email"
    mensaje["From"] = EMAIL
    mensaje["To"] = email
    try: 
        with smtplib.SMTP("smtp.gmail.com", 587) as server: #conecta con el servidor SMTP de gmail en el puerto 587
